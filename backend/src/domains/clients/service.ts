...
export const listClientOrders = async (clientId: string, query: Record<string, unknown>) => {
  ...
  let orders = await enrichOrdersWithLinkedDeliveryMetadata(foundOrders as any[]);
...
export const listClosureGroups = async (
  ...
  const withdrawalOrders = withdrawalIds.length
    ? await OrderModel.find({
      ...
      .lean()
    : [];
  const enrichedOrders = await enrichOrdersWithLinkedDeliveryMetadata(withdrawalOrders as any[]);
